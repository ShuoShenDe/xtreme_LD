...
public class ModelUseCase {
    ...
    @Autowired
    private ImageSAMDatasetInitialInfo imageSAMDatasetInitialInfo;
    ...
    public ModelResponseBO testModelUrlConnection(Long modelId, String url) {
        ...
        switch (modelBO.getModelCode()) {
            ...
            case IMAGE_INTERACTIVE:
                dataInfoBO = dataInfoUseCase.getInitDataInfoBO(imageSAMDatasetInitialInfo);
                System.out.println("dataInfoBO = " + dataInfoBO);
                // 为测试连接创建包含配置数据的 resultFilterParam
                var resultFilterParam = new cn.hutool.json.JSONObject();
                if (imageSAMDatasetInitialInfo.getInteractiveData() != null) {
                    resultFilterParam.set("interactiveData", imageSAMDatasetInitialInfo.getInteractiveData());
                }
                var modelMessage = ModelMessageBO.builder()
                    .dataInfo(dataInfoBO)
                    .resultFilterParam(resultFilterParam)
                    .build();
                var predImageInteractiveReqDTO = ModelSamRequestConverter.convert(modelMessage);
                System.out.println("predImageInteractiveReqDTO = " + predImageInteractiveReqDTO);
                requestBody = JSONUtil.toJsonStr(predImageInteractiveReqDTO);
                break;
            default:
        }
        try {
    ...
    public RecordId sendDataModelMessageToMQ(ModelMessageBO modelMessageBO) {
        ...
        return streamRedisTemplate.opsForStream().add(record);   // 发送到Redis Stream
    }
