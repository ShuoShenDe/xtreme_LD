...
public class UploadDataUseCase {
    ...
    public void commonParseUploadFile(DataInfoUploadBO dataInfoUploadBO, BiConsumer<String, Set<File>> sceneFileListConsumer,
        ...
        if (ObjectUtil.isNotNull(dataInfoUploadBO.getResultType())) {
            ...
            // 检查是否有result文件夹，如果有则设置默认sourceId以支持标注导入
            boolean hasResultFiles = sceneFileList.stream().anyMatch(sceneFile -> 
                FileUtil.loopFiles(sceneFile, 2, null).stream()
                    .anyMatch(fc -> fc.getName().toUpperCase().endsWith(".JSON") 
                        && fc.getParentFile().getName().equalsIgnoreCase("result")));
            if (hasResultFiles) {
                sourceId = -1L;
                log.info("Found result files in upload, setting default sourceId for annotation import");
            }
        }
    ...
    public void handleDataResult(File file, String dataName, DataAnnotationObjectBO dataAnnotationObjectBO,
        ...
        // 先检查是否存在result文件，无论sourceId是否为空
        ...
        // 如果存在result文件，则处理标注数据（无论是否是模型运行结果）
        if (resultFile.isPresent()) {
            // 如果sourceId为空，设置一个默认值以支持普通数据集的标注导入
            if (ObjectUtil.isNull(dataAnnotationObjectBO.getSourceId())) {
                dataAnnotationObjectBO.setSourceId(-1L); // 设置默认sourceId表示手动标注导入
            }
