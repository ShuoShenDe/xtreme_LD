<template>
  <div
    ...
    @click="handleCheck"
  >
    <div class="card-content" :class="{ 'annotated-border': data.annotationStatus === 'ANNOTATED' }">
      <div class="lockInfo">
            ...
            <div class="wrapper-line">
              <div class="openBtn" v-if="props.type != PageTypeEnum.frame">
                <Button
                  ...
                  "
                >
                  {{ data.annotationStatus === 'ANNOTATED' ? 'Saved' : 'Annotate' }}
                </Button>
              </div>
            </div>
          </div>
  ...
  .@{prefix-cls} {
    ...
    .img {
      ...
      .relation-container {
        ...
        overflow: hidden;
        width: 100%;
        height: calc(100% - 38px); // 减去底部标题栏的固定高度
        flex-shrink: 0; // 防止收缩
      }
