...
<script lang="ts" setup>
  ...
  const toolTypeOption = computed(() => {
    if (props.datasetType != datasetTypeEnum.IMAGE) {
      // For LIDAR datasets, show all 3D tools
      return toolTypeList.filter((item) => 
        item.type === ToolTypeEnum.CUBOID ||
        item.type === ToolTypeEnum.POLYLINE_3D ||
        item.type === ToolTypeEnum.POLYGON_3D ||
        item.type === ToolTypeEnum.SEGMENTATION_3D
      );
    } else {
      // For IMAGE datasets, show 2D tools only
      return toolTypeList.filter((item) => 
        item.type !== ToolTypeEnum.CUBOID &&
        item.type !== ToolTypeEnum.POLYLINE_3D &&
        item.type !== ToolTypeEnum.POLYGON_3D &&
