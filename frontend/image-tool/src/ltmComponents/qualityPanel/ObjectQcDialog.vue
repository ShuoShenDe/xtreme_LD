<template>
  <a-modal
    ...
    :title="$t('objectQcDialog.title')"
    :width="500"
    @ok="handleSubmit"
    @cancel="handleCancel"
  >
    <div class="object-qc-dialog">
      <!-- 基本信息和可评价属性 -->
      <div class="info-section" v-if="objectInfo.length > 0">
        <template v-for="(info, index) in objectInfo" :key="index">
          <div class="info-item" v-if="selectedObject && hasProperty(selectedObject, info.propName)">
            <div class="info-label">
              <a-checkbox 
                v-if="info.canCheck && info.valueName"
                v-model:checked="comment.attributes.params!.objectPropChecks![info.valueName]" 
              />
              <span class="label">{{ info.label }}:</span>
            </div>
            <span class="value">{{ getPropertyValue(selectedObject, info.propName) }}</span>
          </div>
        </template>
      </div>

      <!-- 动态渲染组件 -->
      <div v-for="component in objectComponents" :key="component.type" class="component-section">
        <!-- 拒绝原因多选框 -->
        <template v-if="component.type === 'rejectReason'">
          <div class="section-title">{{ $t('objectQcDialog.rejectReason') }}</div>
          <a-select
            v-model:value="comment.attributes.params![component.valueName]"
            mode="multiple"
            :placeholder="$t('objectQcDialog.selectReason')"
            class="reject-select"
        ...
        <template v-if="component.type === 'frameRange'">
          <div class="section-title">{{ $t('objectQcDialog.frameRange') }}</div>
          <a-radio-group 
            v-model:value="comment.attributes.params![component.valueName].type"
            class="frame-range-group"
          >
            <a-radio value="current">{{ $t('objectQcDialog.currentFrame') }}</a-radio>
            <a-radio value="last">{{ $t('objectQcDialog.toLastFrame') }}</a-radio>
            <a-radio value="custom">{{ $t('objectQcDialog.customRange') }}</a-radio>
          </a-radio-group>
          <div 
          ...
          >
            <a-input-number
              ...
              :min="1"
              :max="maxFrameNum"
              :placeholder="$t('objectQcDialog.startFrame')"
              class="frame-input"
            />
            ...
            <a-input-number
              ...
              :max="maxFrameNum"
              :placeholder="$t('objectQcDialog.endFrame')"
              class="frame-input"
            />
          </div>
        </template>
        ...
        <template v-if="component.type === 'comment'">
          <div class="section-title">{{ $t('objectQcDialog.comment') }}</div>
          <a-textarea
            v-model:value="comment.attributes.content"
            :rows="4"
            :placeholder="$t('objectQcDialog.commentPlaceholder')"
    ...
    <template #footer>
      <div class="dialog-footer">
        <a-button @click="handleCancel">
          {{ $t('objectQcDialog.actions.cancel') }}
        </a-button>
        <a-button
          type="primary"
          ...
          @click="handleSubmit"
        >
          {{ $t('objectQcDialog.actions.submit') }}
        </a-button>
      </div>
    </template>
  </a-modal>
...
const { t } = useI18n()
const { createDefaultObjectComment, validateComment, maxFrameNum } = useObjectComment()
const projectMetaStore = useProjectMetaStore()

// 检查对象是否拥有指定属性（兼容嵌套属性和Proxy对象）
const hasProperty = (obj: any, propName: string): boolean => {
  if (!obj) return false
  
  // 处理嵌套属性，如 userData.trackName
...
// 获取对象属性值（兼容嵌套属性和Proxy对象）
const getPropertyValue = (obj: any, propName: string): any => {
  if (!obj) return ''
...
const objectInfo = computed(() => {
  ...
  return projectMetaStore.curQcSettings.annotationComponents?.[configKey as keyof typeof projectMetaStore.curQcSettings.annotationComponents]?.objectInfo || []
})
...
// 内部评论数据
const comment = ref<CommentData>(createDefaultObjectComment())
...
watch(
  ...
  (newComment) => {
    if (newComment) {
      ...
      comment.value = createDefaultObjectComment()
    }
  },
  { immediate: true }
)
...
const isValid = computed(() => {
  const params = comment.value.attributes.params || {}
  ...
  // 检查是否有评论内容
  const hasComment = comment.value.attributes.content.trim() !== ''
