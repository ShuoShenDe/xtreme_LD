...
export function useFrameComment() {
  const projectMetaStore = useProjectMetaStore()
  const commentStore = useCommentStore()
  const { t } = useI18n()
  
  // 创建默认的帧评论
  const createDefaultFrameComment = (): CommentData => {
    // 获取当前帧索引
    ...
    // 根据 QC 设置配置，设置评论的默认值
    const frameComponents = projectMetaStore.curQcSettings.frameComponents
    for (const component of frameComponents) {
  ...
  const validateComment = (comment: CommentData): boolean => {
    ...
    // 检查是否有拒绝原因
    const hasRejectReasons = frameComponents.value.some(component => {
      if (component.type === 'rejectReason') {
  ...
  // 获取帧组件配置
  const frameComponents = computed(() => {
    return projectMetaStore.curQcSettings.frameComponents
  })
  ...
  const frameComments = computed(() => {
    ...
    return commentStore.allComments.data
      .filter(comment => {
        // 过滤条件：属于当前任务、类型为frame、且包含当前帧
        if (comment.attributes.belong !== projectMetaStore.taskId || 
            comment.attributes.type !== 'frame') {
          return false
        }
  ...
  const handleReply = async (payload: { id: string; content: string }) => {
    try {
      const replyComment = createDefaultFrameComment()
