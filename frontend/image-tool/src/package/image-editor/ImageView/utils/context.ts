  ...
  context._stroke = function (shape: Shape) {
    ...
    if (shape.hasStroke()) {
      ...
      if (shape.defaultStyle?.hitStrokeWidth) {
        const hitStrokeWidth = Number(shape.defaultStyle.hitStrokeWidth) * scale;
        // Prevent Infinity values that cause Konva warnings
        if (isFinite(hitStrokeWidth) && hitStrokeWidth > 0) {
          shape.setAttrs({
