...
class SelectHoverAction extends Action {
  ...
  init() {
    ...
    // click - 监听 renderLayer，用于选中气泡等特殊对象
    this.view.renderLayer.on('click', this.onClick);
  }
  destroy() {
    ...
    this.view.renderLayer.off('click', this.onClick);
  }
  ...
  onClick(e: Konva.KonvaEventObject<MouseEvent>) {
    ...
    if (!target.attrs.selectable) {
      return;
    }

    // 检查是否是气泡对象
    if (target.className === 'comment-bubble') {
      // 手动设置气泡的选中状态
      this.view.setState(target, { select: true });
    }
