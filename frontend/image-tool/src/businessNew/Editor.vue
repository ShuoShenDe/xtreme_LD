<template>
  <div class="image-tool-editor">
    ...
    <Mask v-show="false"></Mask>
    <Modal />
    <!-- 评论气泡初始化器 - 虚拟组件，不渲染任何内容 -->
    <CommentBubbleInitializer />
  </div>
</template>
...
<script setup lang="ts">
  ...
  import CommentBubbleInitializer from '@/ltmComponents/qualityPanel/CommentBubbleInitializer.vue';
  ...
  onMounted(async () => {
    ...
    try {
      ...
      trackPerformance('editor_initialized', 1, 'count', {
        timestamp: Date.now(),
        toolType: 'image-tool',
        mode: 'iss_annotation'
      });
    } catch (error) {
      console.warn('Failed to initialize EFFM:', error);
    }
