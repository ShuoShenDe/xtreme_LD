...
export interface IContour {
  ...
  area?: number; // closed figure: rect, polygon, mask pixels
  
  // Multi-Channel Mask for ISS (Instance Semantic Segmentation)
  multiChannelMask?: {
    width: number;
    height: number;
    channels: {
      [channelName: string]: {
        data: number[];           // Channel data
        dataType: 'uint8' | 'uint16' | 'float32';
        description?: string;
      };
    };
    pixelAttributes?: Array<{     // Pixel-level attributes
      id?: number;           
      visible?: boolean;     
      confidence?: number;   
      category?: number;     
