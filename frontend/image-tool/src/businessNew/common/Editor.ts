...
export default class Editor extends BaseEditor {
  ...
  constructor() {
    ...
    // Load language pack data to i18n.messages
    this.i18n.messages = {
      'en-US': languages['en-US'],
      'zh-CN': languages['zh-CN'],
      'ko-KR': languages['ko-KR'],
  ...
  async save() {
    ...
    const { saveDatas } = await utils.getDataFlowSaveData(this, saveFrames);
  ...
  handleInteractive() {
    ...
    const result = this.dataManager.hasModelResult(model.code, frame);
    if (result) {
      this.addInteractiveData(model.code);
      api.clearModel([+frame.id], model.recordId);
    } else {
      this.runInteractive();
    }
  ...
  async addInteractiveData(modelCode: ModelCodeEnum) {
    const frame = this.getCurrentFrame();
    if (!frame || !frame.model) return;
    const { state } = this;
    const data = this.dataManager.getModelResult(modelCode, frame);
    if (!data) return;
    const subTitle = this.lang('Add Interactive Results?');
  ...
  async addInstanceResult(data: any) {
    ...
    const annotates = await utils.convertObject2Annotate(
      this,
      objects.map((o: any) => {
        return {
  ...
  async runModel() {
    ...
    if (!modelConfig.predict && modelConfig.code !== ModelCodeEnum.IMAGE_INTERACTIVE) {
  ...
  async runInteractive() {
    // 这里可以添加interactive模型的具体运行逻辑
    // 暂时复用runModel的逻辑，但可以针对interactive模型进行特殊处理
    const frame = this.getCurrentFrame();
    if (!frame) return;
    
    // 这里可以添加interactive模型的配置和运行逻辑
    
    // 暂时调用runModel方法，后续可以根据需要修改
    this.runModel();
  }
}
