...
export default class Editor extends THREE.EventDispatcher {
    ...
    createAnnotate3D(
        ...
        this.trackAnnotationCreation('cuboid', object, [], { position, scale, rotation });
    ...
    // Extended 3D annotation creation methods
    createAnnotate3DPolyline(points: THREE.Vector3[] = [], userData: IUserData = {}) {
        let object = utils.createAnnotate3DPolyline(this, points, userData);
        utils.setIdInfo(this, object.userData);
        
        this.trackAnnotationCreation('polyline', object, points);
        
        return object;
    }

    createAnnotate3DPolygon(points: THREE.Vector3[] = [], userData: IUserData = {}) {
        let object = utils.createAnnotate3DPolygon(this, points, userData);
        utils.setIdInfo(this, object.userData);
        
        this.trackAnnotationCreation('polygon', object, points);
        
        return object;
    }

    createAnnotate3DSegmentation(points: THREE.Vector3[] = [], indices: number[] = [], userData: IUserData = {}) {
        let object = utils.createAnnotate3DSegmentation(this, points, indices, userData);
        utils.setIdInfo(this, object.userData);
        
        this.trackAnnotationCreation('segmentation', object, points, { indices });
        
        return object;
    }

    createAnnotateByType(objectType: ObjectType, data: any = {}, userData: IUserData = {}) {
        let object = utils.createAnnotateByType(this, objectType, data, userData);
        utils.setIdInfo(this, object.userData);
        return object;
    }

    private trackAnnotationCreation(
        annotationType: 'cuboid' | 'polyline' | 'polygon' | 'segmentation',
        object: any,
        ...
        try {
            // 获取EFFM集成实例（通过全局方式获取）
            const effmIntegration = (window as any).effm;
            
            if (!effmIntegration) {
                            // EFFM integration not available for annotation tracking
                return;
            }

            // Starting annotation tracking
            ...
            switch (annotationType) {
                case 'cuboid':
                    effmIntegration.trackCuboidAnnotation('complete', {
                        ...commonData,
                        position: extraData.position ? {
                            x: extraData.position.x,
                            y: extraData.position.y,
                            z: extraData.position.z
                        } : undefined,
                        dimensions: extraData.scale ? {
                            width: extraData.scale.x,
                            height: extraData.scale.y,
                            depth: extraData.scale.z
                        } : undefined,
                        rotation: extraData.rotation ? {
                            x: extraData.rotation.x,
                            y: extraData.rotation.y,
                            z: extraData.rotation.z
                        } : undefined
                    });
                    break;

                case 'polyline':
                    effmIntegration.trackPolylineAnnotation('complete', {
                        ...commonData,
                ...
                case 'polygon':
                    effmIntegration.trackPolygonAnnotation('complete', {
                        ...commonData,
                        points: points.map(p => ({ x: p.x, y: p.y, z: p.z }))
                    });
                    break;
                ...
                case 'segmentation':
                    effmIntegration.trackSegmentationAnnotation('complete', {
                        ...commonData,
                        points: points.map(p => ({ x: p.x, y: p.y, z: p.z })),
                        indices: extraData.indices || []
                    });
                    break;
            }

            // Successfully tracked annotation creation

        } catch (error) {
            // Failed to track annotation creation
        }
    }
    ...
    updateObjectRenderInfo(objects: AnnotateObject[] | AnnotateObject) {
        ...
        objects.forEach((obj) => {
            ...
            if (obj instanceof Box) {
                ...
                obj.setColor(new THREE.Color(classConfig ? classConfig.color : '#ffffff'));
