...
export default class LoadManager {
    ...
    async loadSceneData(index: number) {
        ...
        if (sceneFrames.length === 0) {
            this.editor.showMsg('warning', `No frames found for scene: ${state.sceneId}`);
            return;
        }
        
        this.editor.setFrames(sceneFrames);

        // Clear current render state to ensure clean scene switch
        this.editor.pc.selectObject(); // Clear selections
        this.editor.pc.clearData(); // Clear point cloud and annotations
        
        // Reset frame index for new scene
        state.frameIndex = -1;
        
        if (state.isSeriesFrame) {
            // load all objects for scene
        ...
        // Load the first frame of the new scene to display the data
        await this.loadFrame(0, true, true);
        
        this.editor.dispatchEvent({ type: Event.SCENE_LOADED });
    }
