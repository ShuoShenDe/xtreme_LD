...
import { AnnotateObject, Box, Rect, Box2D, ITransform, Object2D, Polyline3D, Polygon3D, Segmentation3D } from 'pc-render';
...
interface ITransformPolyline3D {
    points?: THREE.Vector3[];
    closed?: boolean;
}

interface ITransformPolygon3D {
    points?: THREE.Vector3[];
    renderMode?: 'wireframe' | 'filled';
}

interface ITransformSegmentation3D {
    points?: THREE.Vector3[];
    indices?: number[];
    brushRadius?: number;
}

export type IAnnotateTransform = ITransform2DBox | ITransform2DRect | ITransform | ITransformPolyline3D | ITransformPolygon3D | ITransformSegmentation3D;
    ...
    setAnnotatesTransform(
        ...
        objects.forEach((obj, index) => {
            ...
            if (obj instanceof Box) {
                ...
                const polylineData = data as ITransformPolyline3D;
                if (polylineData.points) {
                    obj.points = polylineData.points;
                }
                if (polylineData.closed !== undefined) {
                    obj.closed = polylineData.closed;
                }
            } else if (obj instanceof Polygon3D) {
                const polygonData = data as ITransformPolygon3D;
                if (polygonData.points) {
                    // 使用setPoints方法来正确更新几何体
                    obj.setPoints(polygonData.points);
                }
                if (polygonData.renderMode) {
                    obj.renderMode = polygonData.renderMode;
                }
            } else if (obj instanceof Segmentation3D) {
                const segmentationData = data as ITransformSegmentation3D;
                if (segmentationData.points) {
                    obj.points = segmentationData.points;
                }
                if (segmentationData.indices) {
                    obj.indices = segmentationData.indices;
                }
                if (segmentationData.brushRadius) {
                    obj.brushRadius = segmentationData.brushRadius;
                }
            }
        });
    ...
    loadDataFromManager() {
        ...
        objects.forEach((e) => {
            ...
            if (e instanceof Box) {
                ...
                annotate3D.push(e);
            } else if (e instanceof Polyline3D || e instanceof Polygon3D || e instanceof Segmentation3D) {
                e.parent = this.editor.pc.annotate3D;
                annotate3D.push(e as any);
