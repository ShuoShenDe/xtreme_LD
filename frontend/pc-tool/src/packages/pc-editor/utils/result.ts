...
import { Box, Rect, Box2D, AnnotateObject, Polyline3D, Polygon3D, Segmentation3D } from 'pc-render';
import {
    IUserData,
    IClassType,
    Const,
...
export function translateToObjectV2(object: IObject, baseClassType: IClassType) {
    ...
    // Handle custom contour data for extended 3D types
    const objAsAny = object as any;
    if (objAsAny.contour) {
        if (objAsAny.contour.polylineData) {
            objectV2.contour.polylineData = objAsAny.contour.polylineData;
        }
        if (objAsAny.contour.polygonData) {
            objectV2.contour.polygonData = objAsAny.contour.polygonData;
        }
        if (objAsAny.contour.segmentationData) {
            objectV2.contour.segmentationData = objAsAny.contour.segmentationData;
        }
    }
    return objectV2;
}
export function translateToObject(objectV2: IObjectV2): IObject {
    let object = {
        ...
        objType: objectV2.type || (objectV2 as any)['objType'],
...
export function convertObject2Annotate(objects: IObject[], editor: Editor) {
    ...
    objects.forEach((obj) => {
        ...
        if (objType === ObjectType.TYPE_3D_BOX || objType === ObjectType.TYPE_3D) {
            ...
            bindInfo(box, obj);
            annotates.push(box);
        } else if (objType === ObjectType.TYPE_3D_POLYLINE) {
            // Convert saved point data {x, y, z} back to THREE.Vector3[]
            const savedPoints = obj.contour?.polylineData?.points || [];
            const vector3Points = savedPoints.map((p: any) => new THREE.Vector3(p.x, p.y, p.z));
            const closed = obj.contour?.polylineData?.closed || false;
            
            let polyline = createUtils.createAnnotate3DPolyline(editor, vector3Points, userData);
            polyline.closed = closed; // Set the closed property
            if (classConfig) {
                polyline.color.setStyle(classConfig.color);
            }
            bindInfo(polyline, obj);
            annotates.push(polyline);
        } else if (objType === ObjectType.TYPE_3D_POLYGON) {
            // Convert saved point data {x, y, z} back to THREE.Vector3[]
            const savedPoints = obj.contour?.polygonData?.points || [];
            const vector3Points = savedPoints.map((p: any) => new THREE.Vector3(p.x, p.y, p.z));
            
            let polygon = createUtils.createAnnotate3DPolygon(editor, vector3Points, userData);
            if (classConfig) {
                polygon.color.setStyle(classConfig.color);
            }
            bindInfo(polygon, obj);
            annotates.push(polygon);
        } else if (objType === ObjectType.TYPE_3D_SEGMENTATION) {
            // Convert saved point data {x, y, z} back to THREE.Vector3[]
            const savedPoints = obj.contour?.segmentationData?.points || [];
            const vector3Points = savedPoints.map((p: any) => new THREE.Vector3(p.x, p.y, p.z));
            const indices = obj.contour?.segmentationData?.indices || [];
            
            let segmentation = createUtils.createAnnotate3DSegmentation(
                editor, 
                vector3Points,
                indices,
                userData
            ...
            if (classConfig) {
                segmentation.color.setStyle(classConfig.color);
            }
            bindInfo(segmentation, obj);
            annotates.push(segmentation);
...
export function convertAnnotate2Object(annotates: AnnotateObject[], editor: Editor) {
    ...
    annotates.forEach((obj) => {
        ...
        if (obj instanceof Box) {
            ...
            // Generate contour data for Polyline3D
            info.contour = {
                polylineData: {
                    points: obj.points.map(p => ({ x: p.x, y: p.y, z: p.z })),
                    closed: obj.closed
                }
            };
        } else if (obj instanceof Polygon3D) {
            // Generate contour data for Polygon3D
            info.contour = {
                polygonData: {
                    points: obj.points.map(p => ({ x: p.x, y: p.y, z: p.z }))
                }
            };
        } else if (obj instanceof Segmentation3D) {
            // Generate contour data for Segmentation3D
            info.contour = {
                segmentationData: {
                    points: obj.points.map(p => ({ x: p.x, y: p.y, z: p.z })),
                    indices: obj.indices || []
            ...
            info.viewIndex = parseInt((obj.viewId?.match(/[0-9]{1,5}$/) || ['0'])[0]);
        }
        objects.push(info);
    });
...
function getObjType(annotate: AnnotateObject): ObjectType {
    ...
    else if (annotate instanceof Polyline3D) type = ObjectType.TYPE_3D_POLYLINE;
    else if (annotate instanceof Polygon3D) type = ObjectType.TYPE_3D_POLYGON;
    else if (annotate instanceof Segmentation3D) type = ObjectType.TYPE_3D_SEGMENTATION;
    return type;
}
