...
function hackSideView(editor: Editor, view: SideRenderView) {
    ...
    // Setup point edit integration
    setupPointEditIntegration(editor, view);
}
...
function setupPointEditIntegration(editor: Editor, view: MainRenderView | SideRenderView) {
    // Listen for point edit events from PointEditAction
    view.pointCloud.addEventListener('point-edit', (event: any) => {
        const { object, pointIndex, newPosition } = event.data;
        
        editor.cmdManager.execute('update-points-transform', {
            object,
            pointIndex,
            newPosition
        });
