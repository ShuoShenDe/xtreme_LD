...
import { Object2D, Box, Rect, Vector2Of4, Box2D, AnnotateObject, Polyline3D, Polygon3D, Segmentation3D } from '../objects';
...
export default class Image2DRenderView extends Render {
    ...
    renderObjects() {
        ...
        if (this.renderBox && this.renderPoints && selection3Ds.length > 0) {
            ...
            let selectedObject = selection3Ds[0];
        ...
        if (this.renderBox) {
            object3Ds.forEach((obj) => {
                this.renderBoxData(obj);
            });
            selection3Ds.forEach((obj) => {
                this.renderBoxData(obj);
            });
        }
    }
    ...
    renderBoxData(object: THREE.Object3D) {
        ...
        if (object instanceof Box) {
            ...
            // For other 3D objects, determine color and render
            let color = selectionMap[object.uuid] ? selectColor : object.color;
            let highFlag = this.isHighlight(object);
            color = highFlag ? highlightColor : color;
