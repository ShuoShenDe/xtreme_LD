...
const TypePoints = {
    ...
    'points-3': 3,
    'box-2': 4,
    'points-multi': -1, // -1 means unlimited points
    'brush': -1, // -1 means unlimited points for brush mode
    box: 2,
};
...
export default class CreateAction extends Action {
    ...
    onClick(event: MouseEvent) {
        ...
        // For multi-point modes, check for double-click to complete
        if (isDoubleClick && TypePoints[this.drawType] === -1 && this.points.length >= 2) {
            this.end();
            this.handleCallback();
            return;
        }
    ...
    onKeyDown(event: KeyboardEvent) {
        ...
        // Handle Enter key to complete multi-point operations
        if (event.key === 'Enter' && TypePoints[this.drawType] === -1 && this.points.length >= 2) {
    ...
    handleMouse(event: MouseEvent) {
        ...
        const requiredPoints = TypePoints[this.drawType];
        // For unlimited points types (-1), don't auto-complete
        if (requiredPoints > 0 && this.points.length >= requiredPoints) {
            this.end();
            this.handleCallback();
        }
    }
