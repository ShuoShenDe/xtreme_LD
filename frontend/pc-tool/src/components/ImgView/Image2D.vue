...
<script setup lang="ts">
    ...
    let $$: any;
    let canOperate: any;
    let handleContext: any;
    let clearContext: any;
    let renderProxy: any;
    
    // Initialize dependencies safely
    try {
        editor = useInjectEditor();
        pc = editor.pc;
        state = useInjectState();
        $$ = editor.bindLocale(locale);
        const uiHook = useUI();
        canOperate = uiHook.canOperate;
        const contextHook = useContextMenu();
        handleContext = contextHook.handleContext;
        clearContext = contextHook.clearContext;
        renderProxy = useInjectProxy();
         } catch (error) {
        // Provide fallback values to prevent component crash
        editor = null as any;
        pc = null;
        state = { config: {}, imgViews: [] };
        $$ = (key: string) => key;
        canOperate = () => false;
        handleContext = () => {};
        clearContext = () => {};
    ...
    onMounted(() => {
        ...
        if (!dom.value || !editor || !pc || !state || !renderProxy) {
            return;
        }
        ...
        try {
            ...
            if (state.imgViews && state.imgViews[props.imgIndex]) {
    ...
    onBeforeUnmount(() => {
        try {
            ...
            if (pc && view && typeof view.destroy === 'function') {
                pc.removeRenderView(view);
            }
            
            if (renderProxy && view && typeof renderProxy.removeView === 'function') {
    ...
    function onDBClick() {
        if (!canOperate || !canOperate()) return;
        if (!editor || !editor.viewManager) {
            return;
        }
