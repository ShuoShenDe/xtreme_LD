...
import { AnnotateObject, Box, Rect, Polyline3D, Polygon3D, Segmentation3D } from 'pc-render';
...
export default function useEditClass() {
    ...
    function showObject(trackId: string) {
        ...
        state.resultInstances = tempObjects.map((e) => {
            ...
            let is3D = e instanceof Box || e instanceof Polyline3D || e instanceof Polygon3D || e instanceof Segmentation3D;
            let info = $$('cloud-object');
            if (!is3D) {
                let isRect = e instanceof Rect;
                let index = get2DIndex((e as any).viewId);
                info = $$('image-object', {
                    index: index + 1,
...
function getAnnotateByTrackId(annotates: AnnotateObject[], trackId: string) {
    ...
    annotates.forEach((obj) => {
        ...
        if (obj instanceof Box || obj instanceof Polyline3D || obj instanceof Polygon3D || obj instanceof Segmentation3D) {
            annotate3D.push(obj);
        } else {
            annotate2D.push(obj);
        }
...
function get2DIndex(viewId: string) {
    if (!viewId) return 0; // Handle undefined/null viewId
    const matches = viewId.match(/[0-9]{1,5}$/);
